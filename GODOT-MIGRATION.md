...
- **M0 專案骨架**：project.godot、960x540 視窗、像素渲染設定、資產匯入
- **M1 可動的薩姆斯**：跑步動畫/待機呼吸/背景地形（靜態圖）
- **M2 敵人與生成**：7種雜兵+精英+難度曲線+經驗晶石+升級選單（先文字卡）
- **M3 武器系統**：12武器+8被動+12進化（資料表驅動，行為類別逐一移植）
- **M4 頭目戰**：4巨型中頭目+QUEEN（攻擊模式/死亡演出/連戰照搬）
